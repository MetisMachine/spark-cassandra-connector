  ...
  it should "retry all of our accepted exceptions" in {
    ...
    throwExceptionInPolicy(policy,
      new NoHostAvailableException(
        Map.empty[InetSocketAddress, Throwable].asJava)) should be (retry)

    throwExceptionInPolicy(policy,
      new OverloadedException(new InetSocketAddress(400), "oops")) should be (retry)

    throwExceptionInPolicy(policy,
      new OperationTimedOutException(new InetSocketAddress(400))) should be (retry)
  }
